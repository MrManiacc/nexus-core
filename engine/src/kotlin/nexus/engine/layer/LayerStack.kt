...
interface LayerStack : Collection<Layer<*>> {
    ...
    fun pushLayer(layer: Layer<*>) {
        if (layers.contains(layer)) {
            log.warn("Attempted to add nexus.engine.layer: $layer twice, make sure to pop it first!")
            return
        }
        if (insertIndex + 1 > size)
            insertIndex = size
        layers.add(insertIndex++, layer).also { layer.onAttach() }
            .also { log.info("pushed nexus.engine.layer ${layer.name} to index ${layers.indexOf(layer)}") }
    }
    ...
    fun pushOverlay(layer: Layer<*>) {
        ...
        layers.add(index, layer).also { layer.onAttach() }
            .also { log.info("pushed overlay nexus.engine.layer ${layer.name} to index ${layers.indexOf(layer)}") }
    }

    /*
   This will find the nexus.engine.layer's index and remove it,
then decrement the [insertIndex]
    ...
    fun popLayer(layer: Layer<*>) {
        ...
        if (index != layers.size && index >= 0) {
            ...
            log.info("popped nexus.engine.layer ${layer.name} from index $index")
            insertIndex--
        }
    }

    /*
Removes the last nexus.engine.layer
     */
    fun popOverlay(layer: Layer<*>) {
        ...
        if (index != layers.size && index >= 0) {
            layers.removeAt(layers.indexOf(layer)).also { layer.onDetach() }
                .also {
                    log.info("popped overlay nexus.engine.layer ${layer.name} from index ${layers.size}")
                }
        }
    }
